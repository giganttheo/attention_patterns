...
class VanillaAttentionPattern(AttentionPattern):
  def __init__(self, seq_len_k, seq_len_qv, attention_mask=None, causal=False, n_heads=4, batch_size = 2):
    ...
    if attention_mask is None:
      ...
      seq_qv = [i for i in range(seq_len_qv) if attention_mask[0, i]] #range(seq_len_qv)
      seq_k = range(seq_len_k)#[j for j in range(seq_len_k) if attention_mask[0, j]]
...
def create_dense_attn_patterns(model, max_source_length, max_target_length, n_heads, batch_size, attn_type=VanillaAttentionPattern):
