...
def create_dense_attn_patterns(model, max_source_length, max_target_length, n_heads, batch_size, attention_mask, attn_type=VanillaAttentionPattern):
    ...
    dec_self_attn = attn_type(seq_len_k=max_target_length, seq_len_qv=max_target_length, attention_mask=attention_mask, n_heads=n_heads, batch_size=batch_size, causal=True).get_attention_graph()
