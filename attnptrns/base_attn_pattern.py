...
class AttentionPattern():
  ...
  def get_adj_mat(self, layer_path):
    ...
    for head in range(self.n_heads):
      for i, (r, s) in enumerate(zip(receivers[head], senders[head])):
        if graph_mask[head, i]:
          adj_mat = adj_mat.at[r, s].set(adj_mat[r, s] + (1 / self.n_heads))
    return adj_mat
  ...
  def get_rec_field(self, log=False):
    ...
    rec_field = jax.tree_util.tree_reduce(lambda value, element: value @ element,
                                          adj_mats,
                                          initializer=jnp.eye(self.size[0])
                                          )
    if log:
  ...
  def show_receptive_field(self, log=False):
    rec_field = self.get_rec_field(log=log)
  ...
  def show_attention_pattern(self, layer_path=None):
    ...
    adj_mat = self.get_adj_mat(layer_path)
